 ...
 *
 * @param string $channel Channel pembayaran (contoh: OVO, GoPay, DANA)
 * @param float $amount Jumlah pembayaran
 * @param string $phoneNumber Nomor HP pengguna
 * @param string $walletType Jenis e-wallet (OVO, GoPay, DANA, dll.)
 * @param string $projectName Nama proyek
 * @return string JSON response dari API
 */
function createEwalletPayment($channel, $amount, $phoneNumber, $walletType, $projectName) {
    ...
    $body = [
        ...
        "walletType" => $walletType, // Menambahkan tipe wallet ke request API
        "channel" => $channel,
        "expiryMinutes" => 30
    ];
...
return "test";
    ...
    if (isset($responseData['responseCode']) && $responseData['responseCode'] == 200) {
        $reffId = $responseData['responseData']['id'] ?? 'N/A';
        $amount = $responseData['responseData']['amount'] ?? 0;
        $phoneNumber = $responseData['responseData']['phoneNumber'] ?? 'N/A';
        $channel = $responseData['responseData']['channel'] ?? 'N/A';

        echo "<h2>E-Wallet Payment Generated Successfully</h2>";
        echo "<p>Reff ID: " . htmlspecialchars($reffId) . "</p>";
        echo "<p>Amount: Rp " . number_format($amount, 0, ',', '.') . "</p>";
        echo "<p>Phone Number: " . htmlspecialchars($phoneNumber) . "</p>";
        echo "<p>Channel: " . htmlspecialchars($channel) . "</p>";
    } else {
        $errorMessage = $responseData['responseMessage'] ?? 'Terjadi kesalahan.';
        echo "<p style='color: red;'>Error: " . htmlspecialchars($errorMessage) . "</p>";
    }
